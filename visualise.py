...
def visualise_gan(data_continuous_real, data_continuous_syn, data_discrete_real, data_discrete_syn, inx, max_val_con,
    ...
    for i in range(num_dim):
        ...
        sns.lineplot(ax=ax, data=df_syn.T, palette=sns.color_palette('Reds', n_colors=num_plot), legend=False, alpha=0.3)
        sns.lineplot(ax=ax, data=df_syn.T.mean(axis=1), color='black', linewidth=3, label='Mean')
        ax.set_title(f"Synthetic - {c_title}", fontsize=24)
        ax.set_ylabel("Value", fontsize=20)
        ax.set_xlabel("Time Step (hrs)", fontsize=20)
